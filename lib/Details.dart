...
class _DetailsState extends State<Details> {
  ...
  _menuFetch() async {
    ...
    var res =
        await coll.find({'mainmenu': widget.param, "isActive": 1}).toList();
    if (res == null) {
      _status = 'No data';
    } else {
                            ...
                            if (data[Index]['menuID'] == 'paan') {
                              Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) => Dashboard(
                                      pageIndex: 5,
                                      param: data[Index]['menuID'])));
                            } else {
                              if (data[Index]['menuID'] == 'paann') {
                                Navigator.of(context).push(MaterialPageRoute(
                                    builder: (context) => PanV(param: '', title: '',)));
                              } else {
                                if (data[Index]['mainmenu'] == 'loann') {
                                  Navigator.of(context).push(MaterialPageRoute(
                                      builder: (context) => Dashboard(
                                            title: data[Index]['mainmenu'],
                                            param: data[Index]['menuID'],
                                            pageIndex: 14,
                                          )));
                                } else {
