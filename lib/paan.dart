...
class Paan extends StatefulWidget {
  final String title;
  final String param;
  Paan({Key? key, required this.title, required this.param}) : super(key: key);

  @override
  _PaanState createState() => _PaanState();
}

class _PaanState extends State<Paan> {
  late Timer _timer;
int _index = 0; 
String? _status;
  List<dynamic> data = [];
...
await db.close();
 ...
 _nextpage()async{
  ...
  _menuFetch()async{
   ...
   var res =await coll.findOne({'menuID':'paan','isActive':1});
   if(res == null){
    _status = 'No data';
   }else{
     setState(() {
   ...
   var res3 =await coll3.find({'menuID':'paan','isActive':1}).toList();
   if(res3 == null){
    _status = 'No data';
   }else{
     setState(() {
       ...
       Padding(
         ...
         Container(
          // color:Colors.teal,
           ...
           child: Padding(
             padding: const EdgeInsets.all(8.0),
             child: Text('Apply New Pan Catd',style:TextStyle(color:Colors.white,fontSize: 16),),
           ),),
         ),
         SizedBox(height: 30,),
